...
class UserInterface(object):
    ...
    def create_complete_topic_vocabulary(self, topic_l):
        with open("topic_vocabulary.txt", "r+") as file:
            full_vocabulary = []
            all_lines = set(file)
    ...
    def find_similar_groups(self, user_weight, full_vocab, clusters_ids, accessed_ids, username):
        ...
        for cl_id in clusters_ids:
            ...
            print("Similarity with each group in the selected cluster:", group_similarities_matrix)
            ...
            # CHECK AND APPEND NEW ENTRIES TO ACCESSED IDS!!!!!!!!!!!!!!!!!!!!
            ...
            """
            if set(selected_group_articles) < set(accessed_ids):
                print("Pure match! All read! Let's go to the next more similar group.")
                if set(group_similarities_matrix.index(sorted(group_similarities_matrix, reverse=True)[1])) < set(accessed_ids):
                # append 3rd best group
                    similar_groups.append(group_similarities_matrix.index
                    (sorted(group_similarities_matrix, reverse=True)[2])))
                # else append second best group
                else:
                    similar_groups.append(group_similarities_matrix.index(sorted(group_similarities_matrix, reverse=True)[1]))
            else:
                similar_groups.append(group_similarities_matrix.index(max(group_similarities_matrix)))
            """
        print("\n")
    ...
    # unread_user_articles(self, group_id, n_group, accessed_ids)
